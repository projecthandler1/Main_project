import React from 'react'
import Facebook from '../../images/Facebook.jsx'
import Github from '../../images/Github.jsx'
import Instagram from '../../images/Instagram.jsx'
import Twitter from '../../images/Twitter.jsx'
function Footer() {
  const Elements=[
    {components:<Github/>,link:"https://github.com/"},
    {components:<Facebook/>,link:"https://facebook.com/"},
    {components:<Twitter/>,link:"https://twitter.com/"},
    {components:<Instagram/>,link:"https://instagram.com/"},
]
  return (
    <footer className="relative bg-blueGray-200 pt-8 pb-6">
    <div className="container mx-auto px-4">
    <div className="flex flex-wrap text-left lg:text-left">
      <div className="w-full lg:w-6/12 px-4">
        <h4 className="text-3xl fonat-semibold text-blueGray-700">Let's keep in touch!</h4>
        <h5 className="text-lg mt-0 mb-2 text-blueGray-600">
          Find us on any of these platforms, we respond 1-2 business days.
        </h5>
        <div className="mt-6 lg:mb-0 mb-6">
          {Elements.map((element,i)=>(

          <button className={`${i===1?'bg-black':null} text-white shadow-lg font-normal h-10 w-10 items-center justify-center align-center rounded-full outline-none focus:outline-none mr-2" type="button`} key={i}>
              <div style={{width:'2rem',height:'2rem',paddingLeft:i===1?'10px':null
              ,marginLeft:i===3?'5px':null
            }} >
              <a target="_blank" href={element.link} rel="noreferrer">
            {element.components}
              </a>
              </div>
          </button>
          ))}
         
          
        </div>
      </div>
      <div className="w-full lg:w-6/12 px-4">
        <div className="flex flex-wrap items-top mb-6">
          <div className="w-full lg:w-4/12 px-4 ml-auto">
            <span className="block uppercase text-blueGray-500 text-sm font-semibold mb-2">Useful Links</span>
            <ul className="list-unstyled">
              <li>
                <a className="text-blueGray-600 hover:text-blueGray-800 font-semibold block pb-2 text-sm" href="/About">About Us</a>
              </li>
              <li>
                <a className="text-blueGray-600 hover:text-blueGray-800 font-semibold block pb-2 text-sm" href="/Blog">Blog</a>
              </li>
              <li>
                <a className="text-blueGray-600 hover:text-blueGray-800 font-semibold block pb-2 text-sm" href="https://www.github.com">Github</a>
              </li>
            </ul>
          </div>
          <div className="w-full lg:w-4/12 px-4">
            <span className="block uppercase text-blueGray-500 text-sm font-semibold mb-2">Other Resources</span>
            <ul className="list-unstyled">

              <li>
                <a className="text-blueGray-600 hover:text-blueGray-800 font-semibold block pb-2 text-sm" href="/TermsCondition">Terms &amp; Conditions</a>
              </li>
              <li>
                <a className="text-blueGray-600 hover:text-blueGray-800 font-semibold block pb-2 text-sm" href="/Policies">Privacy Policy</a>
              </li>
              <li>
                <a className="text-blueGray-600 hover:text-blueGray-800 font-semibold block pb-2 text-sm" href="/Contact">Contact Us</a>
              </li>
            </ul>
          </div>
        </div>
      </div>
    </div>
    <hr className="my-6 border-blueGray-300"/>
    <div className="flex flex-wrap items-center md:justify-between justify-center">
      <div className="w-full md:w-4/12 px-4 mx-auto text-center">
        <div className="text-sm text-blueGray-500 font-semibold py-1">
          Copyright © <span id="get-current-year">2022</span><a href="/" className="text-blueGray-500 hover:text-gray-800" target="www.google.com">  Our company</a>
        </div>
      </div>
    </div>
  </div>
</footer>
  )
}

export default Footer